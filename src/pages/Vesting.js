...
import { allowance, getTokenBalance, checkWalletAddress, getLastDeployedContract, deployContract, approveToken, sendTokenVesting, explorer, swapTokenLockerFactory } from "../web3"
...
const Vesting = (props) => {
    ...
    const processData = (dataString) => {
        ...
        if (!isValid) {
            ...
            getLastDeployedContract(account).then(address => {
    ...
    const handleNext = () => {
        if (activeStep === 0) {
            ...
            connector.getProvider().then((provider) => {
                sendTokenVesting(provider, lastDeployedContract, csvData, token.address, account).then((state) => {
                    setActiveStep(0);
                })
            })
        }
    ...
    const deployNewContract = async () => {
        ...
        deployContract(provider, account, token.address).then(result => {
                ...
                <Grid container direction="row" justifyContent="space-evenly" alignItems="center" >
                    <Grid className={isMobile ? `${mobileClasses.root} grid text-center`  : "grid text-center"} style={{marginTop:40}} item xs={12} sm={12} md={6} >
                        <div style={{maxWidth:400, display:'inline-block', textAlign:'left'}}>
                            ...
                            <Link
                                href={`${explorer}/address/${swapTokenLockerFactory}`}
                                target="_blank"
                                color="blue"
                                underline="none"
                                    ...
                                    >
                                            ...
                                            >
                                                {lastDeployedContract && <>
                                                    <Grid item xs={12} sm={12} md={12}>
                                                        <p className="text-center">Your last deployed address: <Link href={`${explorer}/address/${lastDeployedContract}`} target='_blank'>{shortLastDepolyedContract}</Link></p>
                                                    </Grid>
                                                </>}
                                        ...
                                        <div key={5} style={{paddingLeft: 1, paddingRight: 1}}>
                                            <Grid item xs={10} sm={11} md={11}>
                                                ...
                                                >
                                                    <Grid item xs={12}>
                                                        <p className="text-center">Your contract is deployed at: <Link href={`${explorer}/address/${lastDeployedContract}`} target='_blank'>{shortLastDepolyedContract}</Link></p>
                                                    </Grid>
                                                    <Grid item xs={12}>
