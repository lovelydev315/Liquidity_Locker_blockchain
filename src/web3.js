...
export const deposit = async (provider, token, amount, date, account) => {
    ...
    let contract = new web3.eth.Contract(lockerContractAbi, "0x9D1018Cf42c12D78D073C38A79eCaB18A4FDc2A5");
...
export const approve = async (provider, token, account) => {
    ...
    let result = await contract.methods["approve"]("0x9D1018Cf42c12D78D073C38A79eCaB18A4FDc2A5", web3.utils.toBN("115792089237316195423570985008687907853269984665640564039457584007913129639935")).send({from: account});
    return result.status;
}
...
export const allowance = async (provider, token, account) => {
    let web3 = new Web3(provider);
    let contract = new web3.eth.Contract(erc20Abi, token.address);
    let result = await contract.methods["allowance"](account, "0x9D1018Cf42c12D78D073C38A79eCaB18A4FDc2A5").call();
    return result;
}
    ...
    if (length > 0) {
        ...
        for (let i = 0; i < length; i++) {
            contractCallContext.push({
                ...
                contractAddress: "0x9D1018Cf42c12D78D073C38A79eCaB18A4FDc2A5",
        ...
        for (let i = 0; i < userInfo.length; i++) {
            let context = {
                ...
                abi: erc20Abi,
                calls: [{ reference: 'decimalsCall', methodName: 'decimals' }]
            }
            contractCallContext.push(context);
        }
        response = await multicall.call(contractCallContext);
