...
const LockUp = (props) => {
    ...
    const withdrawToken = async (id) => {
        ...
        withdraw(provider, id, account).then(async (status) => {
        ...
        return (
                ...
                >
                    ...
                    <Grid item  xs={12} sm={4} md={3} style={{textAlign:'center'}}>
                        {!isWithdrawable? <Link style={{textDecoration: 'none'}} href={`${explorer}/tx/${event.depositEvent.transactionHash}`} target='_blank' rel='noreferrer'>VIEW TX</Link>: (
                            !isWithdrawn ? (account && account.toLowerCase() === owner.toLowerCase() ? <Button onClick={() => withdrawToken(event.id)} >WITHDRAW</Button> : <Link style={{textDecoration: 'none'}} href={`${explorer}/tx/${event.depositEvent.transactionHash}`} target='_blank' rel='noreferrer'>VIEW TX</Link>) :
                            <Link style={{textDecoration: 'none'}} href={`${explorer}/tx/${event.withdrawEvent.transactionHash}`} target='_blank' rel='noreferrer'>VIEW TX</Link>
                        )}
